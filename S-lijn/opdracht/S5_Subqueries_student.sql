...
SELECT naam FROM medewerkers WHERE mnr IN (SELECT DISTINCT chef FROM medewerkers);
...
SELECT voorl, naam FROM medewerkers WHERE functie = 'TRAINER' AND
        chef IN (SELECT DISTINCT uitvoeringen.docent FROM inschrijvingen
        JOIN uitvoeringen ON inschrijvingen.cursus = uitvoeringen.cursus and inschrijvingen.begindatum = uitvoeringen.begindatum
        JOIN cursussen ON cursussen.code = inschrijvingen.cursus AND cursussen.type = 'ALG');
